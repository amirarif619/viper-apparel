...
export const fetchCart = createAsyncThunk('cart/fetchCart', async(_, { rejectWithValue }) => {
  ...
  if (user) {
    try {
      ...
      const response = await axios.get('viperwearapparel-api.vercel.app/cart', {
        headers: {
...
export const addItem = createAsyncThunk('cart/addItem', async (item, { rejectWithValue }) => {
  ...
  if (user) {
    try {
      const token = await user.getIdToken();
      const response = await axios.post('viperwearapparel-api.vercel.app', item, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export const removeItem = createAsyncThunk('cart/removeItem', async (id, { rejectWithValue }) => {
  ...
  if (user) {
    try {
      const token = await user.getIdToken();
      const response = await axios.delete(`viperwearapparel-api.vercel.appcart/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export const updateItemQuantity = createAsyncThunk('cart/updateItemQuantity', async ({ id, quantity }, { rejectWithValue }) => {
  ...
  if (user) {
    try {
      const token = await user.getIdToken();
      const response = await axios.put(`viperwearapparel-api.vercel.app/cart/${id}`, { quantity }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
